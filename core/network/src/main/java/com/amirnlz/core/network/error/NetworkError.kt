...
sealed class NetworkError(override val message: String? = null) : Throwable(message) {
  ...
  data class ServerError(val code: Int, override val message: String?) : NetworkError("Server error occurred")

  data object Unauthorized : NetworkError("Unauthorized access")
  data class Unknown(override val message: String?) : NetworkError("An unknown network error occurred")
}
